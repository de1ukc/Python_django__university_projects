...
def add_arguments() -> None:
    ...
    group1.add_argument("--source", type=str,
                        help="Файл, из которого будет производиться десериализация")

    group1.add_argument("--destination", type=str,
                        help="Путь к файлу, куда будем сериализовывать")

    parser.add_argument("--mode", type=str, required=True, choices=["Serialize", "Deserialize"],
                        help="Режим работы: сериализация/десериализация")

    parser.add_argument("--type", default="JSON", type=str, choices=["JSON", "YAML", "TOML"],
                        help="Тип, в который будет сериализованы данные")

    parser.add_argument("--destination_type", type=str, required=True, choices=["File", "String"],
